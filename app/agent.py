...
class Agent:
    ...
    def run_tool(self, name: str, args: dict) -> dict:
        ...
        try:
            ...
            return {"error": "not_implemented", "hint": f"The tool {name} is not implemented yet."}
        except Exception as e:
            return {"error": "tool_failed", "hint": f"The tool {name} failed with {type(e).__name__}."}
        ...
        try:
            while True:
                if step_count > MAX_STEPS:
                    raise AgentError("step_limit", "Maximum steps exceeded without an answer.", False)
                ...
                self._log(model_entry)
                if run_id:
                    self.memory.record_model_step(run_id, step_count, turn.tokens_in, turn.tokens_out)
                step_count += 1
                ...
                for call in turn.tool_calls:
                    ...
                    self._log(tool_entry)
                    if run_id:
                        self.memory.record_tool_call(run_id, step_count, call.name, call.args, result, ok, ms)
